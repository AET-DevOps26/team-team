...
public class AccountController {
    ...
    /**
     * Credit utilization as balance / credit limit, the fraction of the credit limit in use.
     * Returns 0 when no positive credit limit is set (avoids division by zero). Replaces a
     * previously hardcoded constant so the figure reflects the account's real balance.
     */
    ...
    public List<BalancePoint> getBalanceTrend(@PathVariable UUID accountId) {
        ...
        for (TransactionItem tx : transactions) {
            ...
            BigDecimal change = "CREDIT".equalsIgnoreCase(tx.direction())
                ? tx.amount()
                : tx.amount().negate();
        ...
        for (int i = 0; i < 6; i++) {
            String monthName = months.get(i).getMonth()
                .getDisplayName(TextStyle.SHORT, Locale.ENGLISH);
